...
def predict_labels(ecg_leads: List[np.ndarray], fs: int, ecg_names: List[str],
    ...
    if not use_pretrained:
        ...
        try:
            state_dict = torch.load("experiments/16_04_2024__22_49_57ECGCNN_M_default_dataset/models/best_model.pt",
                                    map_location=device)
            ...
            print("State dict not found. Download the state dict of ECG-DualNet XL (Icentia11k). "
                  ...
                  "experiments/16_04_2024__22_49_57ECGCNN_M_default_dataset/models/")
            exit(1904)
        ...
        if is_binary_classifier:
            try:
                state_dict = torch.load("experiments/"
                                        "16_04_2024__22_49_57ECGCNN_M_default_dataset/"
            ...
            try:
                state_dict = torch.load("experiments/16_04_2024__22_49_57ECGCNN_M_default_dataset/"
                                        "models/best_model.pt", map_location=device)
            except FileNotFoundError as _:
...
def _train(network: nn.Module, two_classes: bool) -> nn.Module:
    ...
    # Load data
    try:
        ecg_leads, ecg_labels, fs, ecg_names = load_references("./data/validation/")
    ...
    for name, data in zip(ecg_names, dataset):
        ...
        if return_probability:
            if two_classes:
                ...
                predictions.append((name, _get_prediction_name(prediction=prediction_argmax, two_classes=two_classes), max(prediction.reshape(-1).tolist())))
        else:
            predictions.append((name, _get_prediction_name(prediction=prediction_argmax, two_classes=two_classes)))
