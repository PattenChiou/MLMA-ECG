...
def score():
    ...
    if not os.path.exists("data_new/validation/REFERENCE.csv"):
        sys.exit("Es gibt keine Ground Truth")

    df_pred = pd.read_csv("PREDICTIONS.csv", header=None)  # Klassifikationen
    df_gt = pd.read_csv("data_new/validation/REFERENCE.csv", header=None)  # Wahrheit
    ...
    acc = (Nn + Aa + Oo + Pp) / (sum_A + sum_N + sum_O + sum_P)

    return F1, F1_mult, acc


if __name__ == '__main__':
    F1, F1_mult, acc = score()
    print("F1:", F1, "\t MultilabelScore:", F1_mult, "Acc:", acc)
